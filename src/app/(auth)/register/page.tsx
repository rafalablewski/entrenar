    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden"
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute w-[500px] h-[500px] rounded-full opacity-[0.04]"
          style={{ background: "radial-gradient(circle, #A855F7, transparent 70%)", top: "-150px", left: "-100px" }} />
        <div className="absolute w-[400px] h-[400px] rounded-full opacity-[0.03]"
          style={{ background: "radial-gradient(circle, #00F0FF, transparent 70%)", bottom: "-100px", right: "-50px" }} />
      </div>
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            {(["trainer", "athlete"] as const).map((r) => (
              <button
                ...
                type="button"
                onClick={() => setRole(r)}
                className="flex-1 py-3 rounded-xl text-[13px] font-semibold transition-all duration-300"
                style={{
                  background: role === r ? "rgba(0, 240, 255, 0.08)" : "rgba(255,255,255,0.03)",
                  border: role === r ? "1px solid rgba(0, 240, 255, 0.3)" : "1px solid rgba(255,255,255,0.06)",
                  color: role === r ? "#00F0FF" : "rgba(255,255,255,0.4)",
                  boxShadow: role === r ? "0 0 20px rgba(0,240,255,0.08)" : "none",
                }}
              >
          ...
          <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password (min 6 characters)" className="input" />

          {role === "athlete" && (
            <input type="email" required value={trainerEmail} onChange={(e) => setTrainerEmail(e.target.value)} placeholder="Your trainer's email" className="input" />
          )}
        ...
        <p className="text-center text-[13px] mt-8" style={{ color: "rgba(255,255,255,0.3)" }}>
          ...
          <Link href="/login" className="font-medium hover:underline" style={{ color: "#00F0FF" }}>
            Sign in
          </Link>
        </p>
      </div>
